...
def _info_card(title, body_html):
    return (
        '<div class="info-card">'
        f'<h3 class="card-title">{escape(title)}</h3>'
        f"{body_html}"
        "</div>"
    )
...
def _grid(cards):
    return f'<div class="advisory-grid">{"".join(cards)}</div>'
...
def render_advisory(advisory, immediate_remedy=None):
    ...
    html = (
        ...
        f'<span class="status-pill status-info">Urgency: {urgency}</span>'
        f'<h3 class="prediction-name" style="font-size:1.65rem;">{crop} - {disease}</h3>'
        f'<p class="muted-copy">{description}</p>'
        f'<p class="muted-copy"><strong>First action:</strong> {immediate}</p>'
        "</div>"
    )
    ...
    with overview:
        cards = [
            ...
            _info_card(
                ...
                _list_html(advisory.get("environmental_triggers", [])),
            ),
            _info_card("Scientific Note", f"<p class='muted-copy'>{description}</p>"),
        ]
        st.markdown(_grid(cards), unsafe_allow_html=True)

    with treatment:
        cards = [
            _info_card(
                "Treatment Recommendations",
                _list_html(advisory.get("treatment", [])),
            ),
            _info_card(
                "Pesticide Suggestions",
                _list_html(advisory.get("pesticide_suggestions", [])),
            ),
            _info_card(
                "Organic Treatment Options",
                _list_html(advisory.get("organic_treatments", [])),
            ),
            _info_card(
                "Fertilizer Suggestions",
                _list_html(advisory.get("fertilizer_suggestions", [])),
            ),
        ]
        st.markdown(_grid(cards), unsafe_allow_html=True)
    ...
    with prevention:
        cards = [
            _info_card(
                "Prevention Methods",
                _list_html(advisory.get("prevention", [])),
            ),
            _info_card(
                "Early Monitoring",
                _list_html(advisory.get("crop_care_guidance", [])),
            ),
        ]
        st.markdown(_grid(cards), unsafe_allow_html=True)
    ...
    with field_care:
        cards = [
            _info_card(
                "Irrigation Advice",
                _list_html(advisory.get("irrigation_advice", [])),
            ),
            _info_card(
                "Crop-Care Guidance",
                _list_html(advisory.get("crop_care_guidance", [])),
